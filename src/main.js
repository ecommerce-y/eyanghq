const SPEC = {
  ...
  ratios: {
    ...
    diamondCenterX: 0.203125,
    diamondCenterY: 0.408,
    diamondHalfDiag: 0.0975,
    circleRadius: 0.0375,
...
function deriveGeometry(spec) {
  ...
  const junction = { x: header.width, y: header.height };

  const diagonalConst = junction.x + junction.y;
  const leftHit = { x: 0, y: diagonalConst };

  const branchStartX = toPx(width * spec.ratios.branchStartX);
  const branchStart = { x: branchStartX, y: diagonalConst - branchStartX };

  const branchIntercept = branchStart.y - branchStart.x;
  const branchEnd = { x: height - branchIntercept, y: height };
...
function validateConstraints(spec, g) {
